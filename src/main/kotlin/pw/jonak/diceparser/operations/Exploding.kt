...
class Exploding(private val cp: ComparePoint, private val random: Random = Random()) : Operation {
        ...
        fun parse(str: String, sides: Int, random: Random): Pair<Operation?, Int> {
            if (str.isNotEmpty()) {
                if (str.length == 1 && str[0] == '!') {
                    return Pair(Exploding(ComparePoint(Comparison.EQUAL, sides), random), 1)
                }
                if (str.length > 1 && str[0] == '!' && str[1] == '!') {
                    val cpInfo = ComparePoint.parse(str.substring(2))
                    return Pair(
                            Compounding(
                                    cpInfo.first ?: ComparePoint(Comparison.EQUAL, sides),
                                    random
                            ),
                            2 + cpInfo.second
                    )
